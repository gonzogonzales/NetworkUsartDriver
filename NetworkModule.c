...
static struct stm32_port *stm32_of_get_stm32_port(struct platform_device *pdev)
{
	...
	stm32_port.hw_flow_control = of_property_read_bool(np,
							"st,hw-flow-ctrl");
	stm32_port.port.line = id;
	stm32_port.cr1_irq = USART_CR1_RXNEIE;
	stm32_port.cr3_irq = 0;
...
static int rs458_poll(struct napi_struct *napi, int budget)
{
...
static int stm32_serial_probe(struct platform_device *pdev)
{
    ...
    stm32port->ndev = my_net;
    SET_NETDEV_DEV(my_net, &pdev->dev);    
    
    netif_napi_add(my_net, &stm32port->napi, rs458_poll, 64);
